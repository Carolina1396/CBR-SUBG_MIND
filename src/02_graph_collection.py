#import libraries 
from utils.graph_collection_utils import *
import numpy as np
import torch
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_dir_name", default = "00_data/MIND/", help = 'Path to data directory (contains train, test, dev)') 
    parser.add_argument("--data_name", default = "MIND", help = "Name of dataset. Will use it to create output file name")
    parser.add_argument("--knn", default = 5, help = 'Number of KNN to consier for each query')  
    parser.add_argument("--collected_chains_dir", default= "00_data/subgraphs/", help = "Path to directory of collected chains file")
    parser.add_argument("--collected_chains_name", default= "MIND_chains_collection_3hops_1000_paths.pkl", help = "File name of the collected chains")
    parser.add_argument("--branch_size", default = 100, help ="max considered nodes when traversing the graph")
    parser.add_argument("--output_dir", default = "00_data/subgraphs/", help = 'Path to data directory to save collected subgraphs') 
    ...
    splits = ['train00', 'test', 'valid', 'graph'] # Define the list of splits
    ...
    dev_map = load_data(os.path.join(args.data_dir_name, 'valid.txt'))
    ...
    for n, chem in tqdm(enumerate(eval_vocab)):
        nn = get_nearest_neighbor_inner_product(chem,#chem of interest
                                                ...
                                                eval_rev_vocab,  #train vocab index
    ...
    collected_chains_file = os.path.join(args.collected_chains_dir, args.collected_chains_name)
