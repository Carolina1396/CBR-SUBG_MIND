...
def get_nearest_neighbor_inner_product(e1: str, 
                                       nearest_neighbor_1_hop,
                                       eval_vocab, 
                                       rev_entity_vocab,
                                       k: Optional[int] = 5) -> List[str]:
    """
...
def progams_nn (e, nn_list, all_paths_file):
    """
...
def execute_one_program(e: str, path: List[str], depth: int, max_branch: int, train_map):
        ...
        for e_next in next_entities:
            ...
            paths_from_branch = execute_one_program(e_next, path, depth + 1, max_branch)
...
def execute_programs( e: str,
    ...
    for path in path_list:
        ans = execute_one_program(e, path, depth=0, max_branch=max_branch, train_map= train_map) #execute sequence of relations
