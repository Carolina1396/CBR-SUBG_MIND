...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_file", default = "train00.txt", help = "Train file") #triplest to train
    parser.add_argument("--graph_file", default = "graph.txt", help = "Graph file, not include test/dev edges")
    parser.add_argument("--output_dir", default = "00_data/subgraphs/", help = 'Path to data directory to save collected chains')   
    ...
    #import data
    train = pd.read_csv(os.path.join(args.data_dir_name, args.train_file), names= ["start_id", "type", "end_id"], sep ="\t")
    mrn = pd.read_csv(os.path.join(args.data_dir_name, args.graph_file), names= ["start_id", "type", "end_id"], sep ="\t")
